...
export default function RegisterCompanyPage() {
  ...
  function goNext(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.company_name.trim() || !form.sector || !form.country.trim()) {
  ...
  return (
    <div className="auth-page">
      <div className="auth-card auth-card--wide">
        ...
        <p className="auth-subtitle">Set up your agribusiness on CropSight</p>
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} noValidate>
            <div style={{
              ...
              borderRadius: 'var(--radius-sm)', padding: '.65rem .9rem',
              fontSize: '.82rem', color: 'var(--green-900)', marginBottom: '1.25rem',
              display: 'flex', alignItems: 'center', gap: '.5rem'
            }}>
              <span>🏢</span>
              <strong>{form.company_name}</strong>&nbsp;·&nbsp;{form.sector}&nbsp;·&nbsp;{form.country}
              <button type="button" onClick={() => setStep(1)}
                style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'var(--green-800)',
                  cursor: 'pointer', fontSize: '.78rem', fontWeight: 500, textDecoration: 'underline' }}>
                Edit
              </button>
            </div>

            <p className="auth-subtitle" style={{ textAlign: 'left', marginBottom: '1rem', fontSize: '.85rem' }}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                required
              />
              <span className="form-hint">You will be the company administrator</span>
            </div>

            <div style={{ display: 'flex', gap: '.75rem', marginTop: '.5rem' }}>
              <button type="button" className="btn btn--ghost" style={{ flex: '0 0 auto', width: 'auto', padding: '.72rem 1.1rem' }}
                onClick={() => setStep(1)}>
                ← Back
              </button>
